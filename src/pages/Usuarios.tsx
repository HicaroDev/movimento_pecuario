...
function UserModal({ editing, currentUserId, onClose, onSaved }: {
  ...
  const farms = farmService.list().filter(f => f.active);
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<UserFormData>({
    defaultValues: {
      ...
      farmId:  editing?.farmId  ?? (farms[0]?.id || ''),
    },
  });
  ...
  function onSubmit(data: UserFormData) {
    try {
      ...
      if (editing) {
        userService.update(editing.id, payload);
        toast.success('Usuário atualizado!');
      } else {
        if (!data.password) { toast.error('Senha é obrigatória.'); return; }
        userService.create({ ...payload, password: data.password } as Omit<FarmUser, 'id' | 'createdAt'>);
      ...
      <motion.div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10 overflow-hidden"
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
            ...
            <div>
              <label className={labelClass}>
                Senha {editing ? <span className="text-gray-400 font-normal">(vazio = manter)</span> : '*'}
              </label>
            ...
            {/* Fazenda vinculada (só para clientes) */}
            {selectedRole === 'client' && (
              <div>
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-200">
              ...
              <button type="button" onClick={() => setActive(v => !v)}
                ...
                className={`transition-colors ${active ? 'text-teal-600' : 'text-gray-400'} disabled:opacity-40 disabled:cursor-not-allowed`}>
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50">
            ...
            <button type="submit"
              className="flex items-center gap-2 px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold transition-colors">
...
function UserRow({ u, currentUserId, onEdit, onRefresh }: {
  ...
  const farm = u.farmId ? farmService.findById(u.farmId) : null;

  function toggleActive() {
    if (u.id === currentUserId) return;
    userService.update(u.id, { active: !u.active });
    onRefresh();
    ...
    <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      ...
      <td className="px-4 py-3">
        <span className={`text-xs font-bold px-2 py-1 rounded-full ${
          u.role === 'admin' ? 'bg-teal-100 text-teal-700' : 'bg-blue-100 text-blue-700'
        }`}>
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-1">
          <button onClick={() => onEdit(u)} title="Editar"
            className="p-1.5 rounded text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors">
            <Pencil className="w-4 h-4" />
          </button>
          <button onClick={toggleActive} disabled={u.id === currentUserId} title={u.active ? 'Desativar' : 'Ativar'}
            className={`p-1.5 rounded transition-colors disabled:opacity-30 ${
              u.active ? 'text-green-500 hover:bg-green-50' : 'text-red-400 hover:bg-red-50'
            }`}>
            {u.active ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
          </button>
          <button onClick={onDelete} disabled={u.id === currentUserId} title="Excluir"
...
export function Usuarios() {
  ...
  const [users, setUsers]         = useState<FarmUser[]>(() => userService.list());
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing]     = useState<FarmUser | null>(null);

  function refresh() { setUsers(userService.list()); }
  ...
  if (isAdmin) {
    return (
      <div className="min-h-screen bg-gray-50 p-8">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {users.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-4 py-12 text-center text-gray-400">Nenhum usuário cadastrado.</td>
                    </tr>
                  ) : (
                    users.map(u => (
                      <UserRow key={u.id} u={u} currentUserId={user!.id} onEdit={openEdit} onRefresh={refresh} />
                    ))
                  )}
                </tbody>
              </table>
        ...
        <AnimatePresence>
          {modalOpen && (
            <UserModal editing={editing} currentUserId={user!.id} onClose={closeModal} onSaved={refresh} />
          )}
        </AnimatePresence>
      </div>
    );
  ...
  /* ── Cliente: usuários da sua fazenda ── */
  const myFarmId = userService.findById(user?.id || '')?.farmId || '';
  const farmUsers = userService.listByFarm(myFarmId);
  const farm = farmService.findById(myFarmId);
